...
parser.add_argument('--lock-button', type=str, default='right', help='lock按键；只支持鼠标按键')  # TODO
parser.add_argument('--head-first', type=bool, default=False, help='是否优先瞄头')
parser.add_argument('--lock-tag', type=list, default=[1, 0, 3, 2],
...
parser.add_argument('--recoil-button-ak47', type=str, default='left',
...
while True:
    ...
    if args.show_window:
        if args.show_fps:
            cv2.putText(img0, "FPS:{:.1f}".format(1. / (time.time() - t0)) + " yes" if lock_mode else " no", (50, 50),
